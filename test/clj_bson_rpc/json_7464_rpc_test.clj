(ns clj-bson-rpc.json-7464-rpc-test
  (:require
...
(defn- gen-server-client! []
  (let [[a b] (create-duplex-stream)]
    [(connect-json-rpc! a simple-request-handlers {} {:json-framing :rfc-7464})
     (connect-json-rpc! b {:json-framing :rfc-7464})]))
...
(defn- gen-request-handlers [rpc-ctx]
  {:process (fn [msg]
              (doseq [x msg] (notify! rpc-ctx :note (str x)))
              "Done!")
   :echo (fn [msg] (apply str (reverse msg)))})
...
(deftest requests-and-notifications
  (let [record (atom [])
        ...
        ;; services in server
        s (connect-json-rpc! a gen-request-handlers {} {:json-framing :rfc-7464})
        ;; client which accepts notifications from server
        c (connect-json-rpc! b {} (fn [rpc-ctx] (gen-notification-handlers record rpc-ctx))
                             {:json-framing :rfc-7464})]
...
(deftest dynamic-selection-of-requests
  (let [request-handlers (atom {:echo (fn [msg] (apply str (reverse msg)))})
        [a b] (create-duplex-stream)
        [s c] [(connect-json-rpc! a request-handlers {} {:json-framing :rfc-7464})
               (connect-json-rpc! b {:json-framing :rfc-7464})]]
